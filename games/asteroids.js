(function () {
  ...
  function startAsteroids(canvas) {
    ...
    // 720x480 size so content stays crisp when CSS stretches the canvas much
    // larger, e.g. fullscreen. Every existing draw call keeps using the same
    // 720x480 logical space via W/H; ctx.setTransform just makes that space
    // map onto more actual pixels.
    const W = 720;
    const H = 480;
